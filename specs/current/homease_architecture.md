...
| **AI Analysis**            | **Google Gemini & Fal.ai**                                                                                                                                                                          | As per the original specification, these external APIs will be called from Supabase Edge Functions to perform hazard analysis and generate "after" visualizations.                      |
...
```json
{
  ...
  "subtasks": [
    ...
    {
      "description": "Develop the AR assessment flow using a WebXR-based approach for broad accessibility. Create a Supabase Edge Function to securely receive image data from the frontend. This function will orchestrate calls to the Google Gemini API for hazard analysis and the gemini-2.5-flash-image API for generating 'after' visualizations. The results, including the accessibility score, recommendations, and visualization URLs, will be saved to the 'assessments' table in the database.",
...
2. **Presence (Online Status):** This feature tracks which users are currently subscribed to the channel. We will use it to display a "â— Online" or "â— Offline" indicator next to the user's name in the chat header. The client will call `channel.track()` upon joining to announce its presence.
    ...
    <div className="chat-container">
      <div className="chat-header">
        ...
        <span className={isOtherUserOnline ? 'online' : 'offline'}>
          â— {isOtherUserOnline ? 'Online' : 'Offline'}
        </span>
      </div>
